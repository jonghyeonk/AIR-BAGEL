...
class Inject_Anomaly(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        if (self.a == 1 and self.b == 0):
            base_frame_label2.grid(row=0, column=0,columnspan=2,  sticky="nsew", padx=10, pady=10)
            base_frame_label_t.grid(row=2, column=0, columnspan=2,  sticky="nsew", padx=10, pady=10)
        ...
        # Implement system error
            ...
            base_frame_label4.grid(row=0, column=0,columnspan=2,  sticky="nsew", padx=10, pady=10)
            base_frame_label_t.grid(row=2, column=0,columnspan=2,  sticky="nsew", padx=10, pady=10)
            but_implement_sys = tk.Button(base_frame, text="Apply", width=10,
                                          command=lambda: [isok(), seedBox(seedBox2), Abnorm_sys(sys_file=self.system_down, log_file=self.data_with_system, old_file=None).implement_sys_single(df_log=None, df_sys=None,
                                              ...
                                              mag_sys=[self.apply_w_skip_sys(parent), self.apply_w_form_sys(parent), self.apply_w_cut_sys(parent)],
                                              h_skip=self.apply_p_skip_sys(parent), h_form=self.apply_p_form_sys(parent),
                                              h_cut=self.apply_p_cut_sys(parent))])    # system_down = system down log, data_with_system = event log(system)
        ...
        # Implement resource and system error
            ...
            base_frame_label_t.grid(row=3, column=0,columnspan=2,  sticky="nsew", padx=10, pady=10)
            base_frame_label2.grid(row=0, column=0,columnspan=2,  sticky="nsew", padx=10, pady=10)
            base_frame_label4.grid(row=1, column=0,columnspan=2,  sticky="nsew", padx=10, pady=10)
            but_implement = tk.Button(base_frame, text="Apply", width=10,
                                      command=lambda: [isok(), seedBox_two(seedBox1,seedBox2) ,Abnorm_sys(sys_file=self.system_down,
                                          ...
                                          m_rework=self.apply_p_rework(parent), h_skip=self.apply_p_skip_sys(parent),
                                          h_form=self.apply_p_form_sys(parent),
                                          h_cut=self.apply_p_cut_sys(parent))])
        ...
        parent.weight_skip = Text(table11, height=1, width=3)
        ...
        parent.weight_form = Text(table61, height=1, width=3)
        ...
        parent.parameter_form = Text(table62, height=1, width=3)
        parent.parameter_form.grid(row=60, column=20, padx=(3, 151))
        ...
        parent.weight_switch = Text(table21, height=1, width=3)
        ...
        parent.weight_insert = Text(table71, height=1, width=3)
        ...
        parent.parameter_insert = Text(table72, height=1, width=3)
        parent.parameter_insert.grid(row=70, column=20, padx=(3, 151))
        ...
        parent.weight_rework = Text(table51, height=1, width=3)
        ...
        parent.parameter_rework = Text(table52, height=1, width=3)
        parent.parameter_rework.grid(row=50, column=20, padx=(3, 151))
        ...
        parent.weight_moved = Text(table81, height=1, width=3)
        ...
        parent.parameter_moved_Y = Text(table82, height=1, width=3)
        ...
        parent.parameter_moved_m = Text(table82, height=1, width=3)
        ...
        parent.parameter_moved_d = Text(table82, height=1, width=3)
        ...
        parent.parameter_moved_h = Text(table82, height=1, width=3)
        ...
        parent.weight_incom = Text(table41, height=1, width=3)
        ...
        parent.weight_replace = Text(table31, height=1, width=3)
...
#####################################################################################
        ...
        parent.weight_skip_sys = Text(t11, height=1, width=3)
        ...
        parent.parameter_skip_Y_sys = Text(t12, height=1, width=3)
        parent.parameter_skip_Y_sys.grid(row=10, column=30, padx=5)
        parent.parameter_skip_Y_sys.configure(state="disabled", bg="gray60")
        p_skip_m_text_sys = Label(t12, text="m =", font=("Consolas", 10, 'bold'),
                                  fg="white", bg='gray40', anchor="center", width=3)
        p_skip_m_text_sys.grid(row=10, column=35, padx=5)
        parent.parameter_skip_m_sys = Text(t12, height=1, width=3)
        parent.parameter_skip_m_sys.grid(row=10, column=40, padx=5)
        parent.parameter_skip_m_sys.configure(state="disabled", bg="gray60")
        p_skip_d_text_sys = Label(t12, text="d =", font=("Consolas", 10, 'bold'),
                                  fg="white", bg='gray40', anchor="center", width=3)
        p_skip_d_text_sys.grid(row=10, column=45, padx=5)
        parent.parameter_skip_d_sys = Text(t12, height=1, width=3)
        parent.parameter_skip_d_sys.grid(row=10, column=50, padx=5)
        parent.parameter_skip_d_sys.configure(state="disabled", bg="gray60")
        p_skip_h_text_sys = Label(t12, text="h =", font=("Consolas", 10, 'bold'),
                                  fg="white", bg='gray40', anchor="center", width=3)
        p_skip_h_text_sys.grid(row=10, column=55, padx=5)
        parent.parameter_skip_h_sys = Text(t12, height=1, width=3)
        parent.parameter_skip_h_sys.grid(row=10, column=60, padx=5)
        parent.parameter_skip_h_sys.configure(state="disabled", bg="gray60")
        p_skip_M_text_sys = Label(t12, text="M =", font=("Consolas", 10, 'bold'),
                                  fg="white", bg='gray40', anchor="center", width=3)
        p_skip_M_text_sys.grid(row=10, column=65, padx=(5, 5.5))
        parent.parameter_skip_M_sys = Text(t12, height=1, width=3)
        parent.parameter_skip_M_sys.grid(row=10, column=70, padx=4)
        parent.parameter_skip_M_sys.configure(state="disabled", bg="gray60")
        but_skip_sys = ttk.Checkbutton(t10, text="skip", onvalue=1, offvalue=0, variable=parent.cVar_skip_sys,
                                       style='Red.TCheckbutton', width=10, command=lambda: self.able_skip_sys(parent))
        ...
        parent.weight_form_sys = Text(t21, height=1, width=3)
        ...
        parent.parameter_form_Y_sys = Text(t22, height=1, width=3)
        parent.parameter_form_Y_sys.grid(row=20, column=30, padx=5)
        parent.parameter_form_Y_sys.configure(state="disabled", bg="gray60")
        p_form_m_text_sys = Label(t22, text="m =", font=("Consolas", 10, 'bold'),
                                  fg="white", bg='gray40', anchor="center", width=3)
        p_form_m_text_sys.grid(row=20, column=35, padx=5)
        parent.parameter_form_m_sys = Text(t22, height=1, width=3)
        parent.parameter_form_m_sys.grid(row=20, column=40, padx=5)
        parent.parameter_form_m_sys.configure(state="disabled", bg="gray60")
        p_form_d_text_sys = Label(t22, text="d =", font=("Consolas", 10, 'bold'),
                                  fg="white", bg='gray40', anchor="center", width=3)
        p_form_d_text_sys.grid(row=20, column=45, padx=5)
        parent.parameter_form_d_sys = Text(t22, height=1, width=3)
        parent.parameter_form_d_sys.grid(row=20, column=50, padx=5)
        parent.parameter_form_d_sys.configure(state="disabled", bg="gray60")
        p_form_h_text_sys = Label(t22, text="h =", font=("Consolas", 10, 'bold'),
                                  fg="white", bg='gray40', anchor="center", width=3)
        p_form_h_text_sys.grid(row=20, column=55, padx=5)
        parent.parameter_form_h_sys = Text(t22, height=1, width=3)
        parent.parameter_form_h_sys.grid(row=20, column=60, padx=5)
        parent.parameter_form_h_sys.configure(state="disabled", bg="gray60")
        p_form_M_text_sys = Label(t22, text="M =", font=("Consolas", 10, 'bold'),
                                  fg="white", bg='gray40', anchor="center", width=3)
        p_form_M_text_sys.grid(row=20, column=65, padx=(5, 5.5))
        parent.parameter_form_M_sys = Text(t22, height=1, width=3)
        parent.parameter_form_M_sys.grid(row=20, column=70, padx=4)
        parent.parameter_form_M_sys.configure(state="disabled", bg="gray60")
        but_form_sys = ttk.Checkbutton(t20, text="form based", onvalue=1, offvalue=0, variable=parent.cVar_form_sys,
                                       style='Red.TCheckbutton', width=10, command=lambda: self.able_form_sys(parent))
        ...
        parent.weight_cut_sys = Text(t31, height=1, width=3)
        ...
        parent.parameter_cut_Y_sys = Text(t32, height=1, width=3)
        parent.parameter_cut_Y_sys.grid(row=30, column=30, padx=5)
        parent.parameter_cut_Y_sys.configure(state="disabled", bg="gray60")
        p_cut_m_text_sys = Label(t32, text="m =", font=("Consolas", 10, 'bold'),
                                 fg="white", bg='gray40', anchor="center", width=3)
        p_cut_m_text_sys.grid(row=30, column=35, padx=5)
        parent.parameter_cut_m_sys = Text(t32, height=1, width=3)
        parent.parameter_cut_m_sys.grid(row=30, column=40, padx=5)
        parent.parameter_cut_m_sys.configure(state="disabled", bg="gray60")
        p_cut_d_text_sys = Label(t32, text="d =", font=("Consolas", 10, 'bold'),
                                 fg="white", bg='gray40', anchor="center", width=3)
        p_cut_d_text_sys.grid(row=30, column=45, padx=5)
        parent.parameter_cut_d_sys = Text(t32, height=1, width=3)
        parent.parameter_cut_d_sys.grid(row=30, column=50, padx=5)
        parent.parameter_cut_d_sys.configure(state="disabled", bg="gray60")
        p_cut_h_text_sys = Label(t32, text="h =", font=("Consolas", 10, 'bold'),
                                 fg="white", bg='gray40', anchor="center", width=3)
        p_cut_h_text_sys.grid(row=30, column=55, padx=5)
        parent.parameter_cut_h_sys = Text(t32, height=1, width=3)
        parent.parameter_cut_h_sys.grid(row=30, column=60, padx=5)
        parent.parameter_cut_h_sys.configure(state="disabled", bg="gray60")
        p_cut_M_text_sys = Label(t32, text="M =", font=("Consolas", 10, 'bold'),
                                 fg="white", bg='gray40', anchor="center", width=3)
        p_cut_M_text_sys.grid(row=30, column=65, padx=(5, 5.5))
        parent.parameter_cut_M_sys = Text(t32, height=1, width=3)
        parent.parameter_cut_M_sys.grid(row=30, column=70, padx=4)
        parent.parameter_cut_M_sys.configure(state="disabled", bg="gray60")
    ...
    def able_skip_sys(self, parent):
        if parent.cVar_skip_sys.get() == 1:
            ...
            parent.parameter_skip_Y_sys.configure(state="normal", bg="white")
            parent.parameter_skip_m_sys.configure(state="normal", bg="white")
            parent.parameter_skip_d_sys.configure(state="normal", bg="white")
            parent.parameter_skip_h_sys.configure(state="normal", bg="white")
            parent.parameter_skip_M_sys.configure(state="normal", bg="white")
    ...
    def able_form_sys(self, parent):
        if parent.cVar_form_sys.get() == 1:
            ...
            parent.parameter_form_Y_sys.configure(state="normal", bg="white")
            parent.parameter_form_m_sys.configure(state="normal", bg="white")
            parent.parameter_form_d_sys.configure(state="normal", bg="white")
            parent.parameter_form_h_sys.configure(state="normal", bg="white")
            parent.parameter_form_M_sys.configure(state="normal", bg="white")
    ...
    def able_cut_sys(self, parent):
        if parent.cVar_cut_sys.get() == 1:
            ...
            parent.parameter_cut_Y_sys.configure(state="normal", bg="white")
            parent.parameter_cut_m_sys.configure(state="normal", bg="white")
            parent.parameter_cut_d_sys.configure(state="normal", bg="white")
            parent.parameter_cut_h_sys.configure(state="normal", bg="white")
            parent.parameter_cut_M_sys.configure(state="normal", bg="white")
    ...
    def apply_p_skip_sys(self, parent):
        if parent.cVar_skip_sys.get() == 1:
            if parent.parameter_skip_Y_sys.get("1.0", "end-1c") == "":
                Y = 0
            else:
                Y = int(parent.parameter_skip_Y_sys.get("1.0", "end-1c"))
            if parent.parameter_skip_m_sys.get("1.0", "end-1c") == "":
                m = 0
            else:
                m = int(parent.parameter_skip_m_sys.get("1.0", "end-1c"))
            if parent.parameter_skip_d_sys.get("1.0", "end-1c") == "":
                d = 0
            else:
                d = int(parent.parameter_skip_d_sys.get("1.0", "end-1c"))
            if parent.parameter_skip_h_sys.get("1.0", "end-1c") == "":
                h = 0
            else:
                h = int(parent.parameter_skip_h_sys.get("1.0", "end-1c"))
            if parent.parameter_skip_M_sys.get("1.0", "end-1c") == "":
                M = 0
            else:
                M = int(parent.parameter_skip_M_sys.get("1.0", "end-1c"))
    ...
    def apply_p_form_sys(self, parent):
        if parent.cVar_form_sys.get() == 1:
            if parent.parameter_form_Y_sys.get("1.0", "end-1c") == "":
                Y = 0
            else:
                Y = int(parent.parameter_form_Y_sys.get("1.0", "end-1c"))
            if parent.parameter_form_m_sys.get("1.0", "end-1c") == "":
                m = 0
            else:
                m = int(parent.parameter_form_m_sys.get("1.0", "end-1c"))
            if parent.parameter_form_d_sys.get("1.0", "end-1c") == "":
                d = 0
            else:
                d = int(parent.parameter_form_d_sys.get("1.0", "end-1c"))
            if parent.parameter_form_h_sys.get("1.0", "end-1c") == "":
                h = 0
            else:
                h = int(parent.parameter_form_h_sys.get("1.0", "end-1c"))
            if parent.parameter_form_M_sys.get("1.0", "end-1c") == "":
                M = 0
            else:
                M = int(parent.parameter_form_M_sys.get("1.0", "end-1c"))
    ...
    def apply_p_cut_sys(self, root):
        if root.cVar_cut_sys.get() == 1:
            if parent.parameter_cut_Y_sys.get("1.0", "end-1c") == "":
                Y = 0
            else:
                Y = int(parent.parameter_cut_Y_sys.get("1.0", "end-1c"))
            if parent.parameter_cut_m_sys.get("1.0", "end-1c") == "":
                m = 0
            else:
                m = int(parent.parameter_cut_m_sys.get("1.0", "end-1c"))
            if parent.parameter_cut_d_sys.get("1.0", "end-1c") == "":
                d = 0
            else:
                d = int(parent.parameter_cut_d_sys.get("1.0", "end-1c"))
            if parent.parameter_cut_h_sys.get("1.0", "end-1c") == "":
                h = 0
            else:
                h = int(parent.parameter_cut_h_sys.get("1.0", "end-1c"))
            if parent.parameter_cut_M_sys.get("1.0", "end-1c") == "":
                M = 0
            else:
                M = int(parent.parameter_cut_M_sys.get("1.0", "end-1c"))
